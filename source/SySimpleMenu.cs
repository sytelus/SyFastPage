		...
		public class SySimpleMenu : System.Web.UI.WebControls.WebControl
		{
			...
			public String BuildMenu()
			{
				...
				if (oMenuInfo == null)
				{
					...
					sDisabledMenuHtml = ExtractNavigationHtml(oNavigationXmlDocument, "disabledMenuHtml", String.Empty);
					...
					oMenuInfo.Add("DisabledMenuHtml", sDisabledMenuHtml);
				...
				else
				{
					...
					sDisabledMenuHtml = (String) oMenuInfo["DisabledMenuHtml"];
				}
				...
				foreach(DataRowView oParentMenuRow in oParentMenuView)
				{
					...
					sThisParentMenuHtml = BuildBasicHtmlForMenuItem(oParentMenuRow,sParentMenuNormalHtml, 
						(bIsActiveMenuAlreadyFound==true)?sParentMenuNormalHtml:sParentMenuActiveHtml, sDisabledMenuHtml, true, ref bIsThisParentMenuActive);
					...
					else
					{
						...
						{
							sThisParentMenuHtml = CommonFunctions.ReplaceString(sThisParentMenuHtml, "[sub_menu_active_indicator]", sSubMenuIndicatorActiveHtml, false);
							sThisParentMenuHtml = CommonFunctions.ReplaceString(sThisParentMenuHtml, "[sub_menu_normal_indicator]", String.Empty, false);
							...
							foreach(DataRowView oSubMenuRow in oSubMenuView)
							{
								...
								sSubMenuInnerHtml += BuildBasicHtmlForMenuItem(oSubMenuRow,sSubMenuNormalHtml,sSubMenuActiveHtml, sDisabledMenuHtml, false,ref bIsThisSubMenuActive);	
							}
						...
						{
							sThisParentMenuHtml = CommonFunctions.ReplaceString(sThisParentMenuHtml, "[sub_menu_active_indicator]", String.Empty, false);
							sThisParentMenuHtml = CommonFunctions.ReplaceString(sThisParentMenuHtml, "[sub_menu_normal_indicator]", sSubMenuIndicatorNormalHtml, false);

							//If parent menu is not active then hide submenus
							sThisParentMenuHtml = CommonFunctions.ReplaceString(sThisParentMenuHtml, "[sub_menu_html]", String.Empty, false);
						}
					}
			...
			private string BuildBasicHtmlForMenuItem(DataRowView voMenuItemDatarow, string vsNormalMenuTemplate, 
				string vsActiveMenuTemplate, string vsDisabledMenuTemplate, bool vbIsParentMenu, ref bool rbIsThisMenuActive)
			{
				...
				bool bIsMenuDisabled = false;

				if (voMenuItemDatarow.Row.Table.Columns.Contains("isDisabled") == false)
				{
					bIsMenuDisabled = false;
				}
				else if (voMenuItemDatarow["isDisabled"] == DBNull.Value )
				{
					bIsMenuDisabled = false;
				}
				else
				{
					bIsMenuDisabled = bool.Parse(voMenuItemDatarow["isDisabled"].ToString());
				}

				bool bIsThisMenuActive = IsActiveMenu(sMenuName, sMenuUrl, vbIsParentMenu);
				string sThisMenuHtml;
				if (bIsMenuDisabled == true)
				{
					sThisMenuHtml = vsDisabledMenuTemplate;
				}
				else if (bIsThisMenuActive==true)
				{
